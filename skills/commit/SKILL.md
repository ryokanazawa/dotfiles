...
# /commit — changelog・コミット・main反映・push

今セッションの変更だけをコミットする。ユーザー向け変更なら `CHANGELOG.md`（またはリポジトリの changelog 相当）を更新する。worktree上のコミットは即mainへマージし、mainをpushしてからpullで同期を確認する。
...
### 2. changelogを更新する（必要なとき）

次のいずれかに当てはまる変更は、コミット前にchangelogへ追記する。
