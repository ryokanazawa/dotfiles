...
mainチェックアウトが存在しない場合は、現在の作業ブランチ名（detached HEADならその状態）とコミットSHAを記録してから現在地で`git switch main`し、ここをmainチェックアウトとして扱う。以降、作業ブランチがあればそのブランチ、detached HEADなら記録したSHAをrebase対象に指定する。ローカルmainがなく`origin/main`が存在する場合は、`git switch --track origin/main`で作成する。どちらも存在しなければ統合先を確認する。
...
2. 作業worktree（detached HEADを含む）で`git rebase main`を実行する。同じチェックアウトをmainへ切り替えた場合は`git rebase main <作業ブランチまたは記録したSHA>`を使い、rebase後の先端SHAを記録して`git switch main`で戻る。コンフリクトしたら範囲内で解消し、関連検証とautoreviewを行ってから`git rebase --continue`する。解消できなければ`git rebase --abort`で戻して停止する。rebase後の各コミットSHAを記録し直す。
